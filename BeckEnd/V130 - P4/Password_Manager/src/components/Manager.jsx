...
const Manager = () => {
    ...
    const savePassword = () => {
        ...
        try {
            ...
            // setPasswordLog([null])
            localStorage.setItem('password', JSON.stringify([...passwordLog, { ...form, id: uuidv4() }]))
    ...
    const deletePass = (id) => {
        if (confirm('Are You Sure You want to delete this Password?')) {
            setPasswordLog(passwordLog.filter(item => item.id !== id))
            localStorage.setItem('password', JSON.stringify(passwordLog.filter(item => item.id !== id)))
        }
    }
